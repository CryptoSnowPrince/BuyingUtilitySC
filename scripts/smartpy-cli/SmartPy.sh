...
usage () {
    ...
    echo
    echo "   Protocols: ithaca | jakarta | kathmandu"
}
...
case $action in
    ...
    "regular" )
        ...
        if [[ $has_mockup == yes ]]; then
            $smartpyc "$script" --kind $kind --output "$output" --install $install_path --mockup "$@"
